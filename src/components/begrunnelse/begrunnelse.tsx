...
const Begrunnelse = (props: any) => {
  ...
  const [activeBegrunnelse, setActiveBegrunnelse] = useState<string>(
    activeKlageSkjema.fritekst ?? ""
  );
  const [activeDatoISO, setActiveDatoISO] = useState<string>(
    activeKlageSkjema.vedtaksdatoobjekt
      ? toISOString(activeKlageSkjema.vedtaksdatoobjekt)
      : ""
  );
  const [datoalternativ, setDatoalternativ] = useState<string>(
    activeKlageSkjema.datoalternativ ?? ""
  );
  ...
  const getFeilmeldinger = (): string[] => {
    ...
    if (!validDatoalternativ()) {
      feilmeldinger.push(
        "Du må velge hvilket vedtak du ønsker å klage på før du går videre."
      );
    }
    if (!validBegrunnelse()) {
    ...
    <>
      ...
      <MarginContainer>
        <RadioPanelGruppe
          ...
          onChange={(event: any, value: string) =>
            handleDatoalternativClick(event, value)
          }
          feil={
            submitted &&
            !validDatoalternativ() &&
      ...
      <Margin48Container className="override-overlay">
        ...
        <Textarea
          ...
          feil={
            submitted &&
            !validBegrunnelse() &&
            "Du må skrive en begrunnelse før du går videre."
          }
        />
      </Margin48Container>
      ...
      <MarginContainer>
        ...
        <VedleggVisning
          ...
          deleteAction={(vedlegg) => removeAttachment(vedlegg)}
        />
        {vedleggLoading && (
          <CenteredContainer>
        ...
        <MarginTopContainer className="override-overlay">
          <Normaltekst>
            Om du har ny eller oppdatert informasjon du ønsker å legge ved kan
            det lastes opp her.
          </Normaltekst>
          <MarginTopContainer>
            <Normaltekst>
              All informasjon du har sendt inn tidligere i denne saken vil følge
              med klagen din og trenger ikke lastes opp på nytt.
            </Normaltekst>
          </MarginTopContainer>
        </MarginTopContainer>
      ...
      <Margin40Container>
        <Knapp onClick={(e) => handleAttachmentClick(e)}>
      ...
      <MarginContainer>
        <AlertStripe type="info" form="inline">
          ...
          <Undertekst>
            Filstørrelsen kan ikke være større enn 8 MB, og total størrelse av
            alle vedlegg kan ikke være større enn 32 MB.
          </Undertekst>
        </AlertStripe>
      </MarginContainer>
      ...
      <Margin48TopContainer className="override-overlay">
        <FlexCenteredContainer>
          <Hovedknapp
            ...
            onClick={(event: any) => submitBegrunnelseOgDato(event)}
          >
            Gå videre
          </Hovedknapp>
        </FlexCenteredContainer>
