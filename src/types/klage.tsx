...
export const klageSkjemaTilKlage = (klageskjema: KlageSkjema): Klage => {
  const getVedtaksDato = (): string => {
    let result = "";
    let foundDatoAlternativ = datoValg.find(
      (valg) => valg.value === klageskjema.datoalternativ
    );
    ...
    if (foundDatoAlternativ !== undefined && vedtaksdatoobjekt) {
      result +=
        ...
        (foundDatoAlternativ.id === "tidligereVedtak"
          ? " - " + formatDate(vedtaksdatoobjekt)
          : "");
    } else if (vedtaksdatoobjekt) {
      result +=
        datoValg.find((valg) => valg.id === "tidligereVedtak")?.value +
        " - " +
        formatDate(vedtaksdatoobjekt);
    }
...
const getVedtaksDatoObjekt = (vedtak: string) => {
  if (
    vedtak.startsWith("Tidligere vedtak -") &&
    vedtak !== "Tidligere vedtak - Ingen dato satt"
  ) {
